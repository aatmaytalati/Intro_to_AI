...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for Distribution in LGPD:
            
            maxDist = -3
            for key, vl in Distribution.items():
                if ((vl >maxDist)):
                    maxDist = vl
                    maxKey = key
            if((maxDist == 0) or (maxDist >0)):
                AP.append(maxKey)
     
        AGD = [(self.distancer.getDistance(pcp, pos), pos) for pos in AP]
        CGP = min(AGD)[1]
        mvs = [(self.distancer.getDistance(Actions.getSuccessor(pcp, mv), CGP), mv) for mv in legal]
        acts = [minDist[1] for minDist in mvs if minDist[0] == min(mvs)[0]]
        return random.choice(acts)

        # util.raiseNotDefined() <- No longer needed. - AT