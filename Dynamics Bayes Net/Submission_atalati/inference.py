...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for ps in SB:
            if SB[ps] > 0:
                self.setGhostPosition(gameState, ps)
                GPD = self.getPositionDistribution(gameState)
                
                for lp in self.legalPositions:
                    temp1 = GPD[lp]
                    temp2 = SB[ps]
                    newBeliefs[lp] += temp2 * temp1
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        legalPositions = [lp for lp in self.legalPositions]
        
        self.particles = []
        numPart, counterLP = 0, 0
        
        while numPart != self.numParticles:
            boo = counterLP != len(self.legalPositions)
            counterLP *= boo # I am testing something I learned in CompArch
            # if counterLP == len(self.legalPositions): #above statement is equivalent to if statement
            #     counterLP = 0
            self.particles.append(self.legalPositions[counterLP])
            numPart += 1
            counterLP += 1
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance != None:
            for prt in self.particles:
                trueDistance = util.manhattanDistance(prt, pacmanPosition)
                temp1 = emissionModel[trueDistance]
                allPossible[prt] += temp1
        else:
            allPossible[self.getJailPosition()] = 1.0
        ...
        for i in range(self.numParticles):
            newP.append(util.sample(allPossible) if boo else random.choice(self.legalPositions))
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        newPs = [util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))) for oldPos in self.particles]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        ALP = itertools.product(self.legalPositions, repeat = self.numGhosts)
        legalPos = [lp for lp in ALP]
        ...
        while numPart != self.numParticles:
            boo = counterLP != len(legalPos)
            counterLP *= boo
            self.particles.append(legalPos[counterLP])
            numPart += 1
            counterLP += 1
        ...
        for part in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                if noisyDistances[i] == None:
                    ...
                    tmp *= emissionModels[i][util.manhattanDistance(pacmanPosition, gost)]
            
            allPossible[part] += tmp
        
        dst = [dist for _, dist in allPossible.items()]
        
        if not (allPossible.totalCount() or sum(dst)):
            self.initializeParticles()
            return

        newParticles = [util.sample(allPossible) for i in range(self.numParticles)]        
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            for gost in range(self.numGhosts):
                SGA = self.ghostAgents[gost]
                SGP = setGhostPositions(gameState, list(oldParticle))
                GPDFG = getPositionDistributionForGhost(SGP, gost, SGA)
                newParticle[gost] = util.sample(GPDFG)
